...
app.directive('pageview', ['$rootScope',
    ...
    'MessageService',
	'ThemeService',
    'MessageManager', function($rootScope, $timeout, $compile, Core, Security, LockManager, HttpRequeset, MessageService, ThemeService, MessageManager) {
...
//            	console.log("<"+$element[0].tagName+"> attribute of page-records-limit default as 10");
                MessageManager.L(MessageManager.GetMessageText("PageRecordsLimitDefault", "10"), $element[0].tagName, "PageRecordsLimitDefault");
...
//            console.log("scope.$id:"+$scope.$id+", may implement $scope.EventListener() function in webapge");
            MessageManager.L(MessageManager.GetMessageText("PageviewEventListener", $scope.$id), $element[0].tagName, "PageviewEventListener");
        }
        function ValidateRecord(){
//            console.log("scope.$id:"+$scope.$id+", may implement $scope.ValidateRecord() function in webapge");
            MessageManager.L(MessageManager.GetMessageText("PageviewValidateRecord", $scope.$id), $element[0].tagName, "PageviewValidateRecord");
            return true;
        }
...
//                console.log("<"+$element[0].tagName+">" +" Directive function CustomPointedToRecord() should be override.");
                MessageManager.L(MessageManager.GetMessageText("CustomPointedToRecordNotFound", $scope.programId), $element[0].tagName, "CustomPointedToRecordNotFound");
            }
        }
...
//                console.log("<"+$element[0].tagName+">" +" Directive function CustomSelectedToRecord() should be override.");
                MessageManager.L(MessageManager.GetMessageText("CustomSelectedToRecordNotFound", $scope.programId), $element[0].tagName, "CustomSelectedToRecordNotFound");
            }
...
//    			console.log("This is the first page, cannot go previous.")
                MessageManager.L(MessageManager.GetMessageText("BeginningOfThePageCannotGotoPrevious", $scope.programId), $element[0].tagName, "BeginningOfThePageCannotGotoPrevious");
    		}
    	}
...
//    		console.log("Going to display the Page no.("+pageNum + ") records.");
            MessageManager.L(MessageManager.GetMessageText("DisplayPageNum", $scope.programId, pageNum), $element[0].tagName, "DisplayPageNum");
    		var numOfRecordPerPage = $scope.numOfRecordPerPage;
...
app.directive('entry', ['$rootScope',
    ...
    'HttpRequeset',
    'MessageService',
    'ThemeService', function($rootScope, $q, $timeout, $compile, Core, Security, LockManager, LoadingModal, HttpRequeset, MessageService, ThemeService) {
...
//            deleteObj.Header[1] = ConvertKeyFieldToUppercase(recordObj, true);
    ...
    function templateFunctionOLD(tElement, tAttrs) {
        var globalCriteria = $rootScope.globalCriteria;
 ...
 */
app.directive('screen', ['Security', '$rootScope', '$compile', function(Security, $rootScope, $compile) {
 ...
 */
app.directive('editbox', ['Security', '$rootScope', '$compile', 'ThemeService', 'config', function(Security, $rootScope, $compile, ThemeService, config) {
		        ...
		        pre: function preLink(scope, iElement, iAttrs, controller) {
                    console.log("3 Edotbox - compile preLink()");
                        transclude(scope, function(clone, scope) {
                            ...
                            var linkFn = $compile(clone);
                            var pageviewElement = linkFn(scope);

                            iElement.find('.custom-transclude').append(clone);
                            // iElement.find('ng-transclude').append(clone);
                        });
		        },
		        post: function postLink(scope, iElement, iAttrs, controller) {
...
app.directive('message', ['$rootScope',
    ...
    'Security',
    'MessageService',
    'ThemeService', function($rootScope, $timeout, Security, MessageService, ThemeService) {
